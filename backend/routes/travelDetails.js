...
const { createRouter: createDecisionRouter } = require('../services/router');
...
const routerDecision = createDecisionRouter({ enableBrowserEscalation: true });

const orchestrator = createOrchestrator({
  tools: [
    {
      name: 'generateTravelPackage',
      description: 'Run API layer + subagents + fusion to build a full trip package.',
      execute: async (args = {}) => {
    ...
    {
      name: 'ragSearch',
      description: 'Search RAG store for similar past plans, preferences, and context.',
      execute: async (args = {}) => {
    ...
    },
    {
      ...
      description: 'Fallback to browser automation when APIs fail or live data is required.',
      execute: async (args = {}) => {
...
// ─── Monitor bridge (fire-and-forget) ─────────────────────────────────
let _emitEvt;
try {
  _emitEvt = require('../services/monitorBridge').emitEvent;
} catch {
  _emitEvt = null;
}
function emitStep(layer, event, data, sessionId) {
  if (!_emitEvt) return;
  try { _emitEvt(layer, event, data, sessionId || 'default'); } catch {}
}
 ...
 * Returns planning progress logs for a given session.
 */
...
router.post('/plan', async (req, res) => {
  ...
  try {
    ...
    emitStep('orchestrator', 'generate_completed', {
      ...
      toolPlan: orchestratorResult?.toolPlan?.toolCalls?.map(t => t.toolName || t.name) || [],
 ...
 * Fetch detailed data for a specific tab type (hotels, travel, places, food).
 */
